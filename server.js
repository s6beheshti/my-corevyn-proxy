...
و `package.json` را هم این کن:

```json
{
  "name": "corevyn-proxy",
  "version": "2.0.0",
  "description": "Corevyn market data proxy",
  "main": "server.js",
  "scripts": {
...
### حالا دقیقاً چه کار کن

در GitHub فقط این دو فایل را داشته باش:
...
Render با هر Push به branch متصل‌شده می‌تواند دوباره build/deploy کند.
...
و نیازی نیست `PORT=3000` دستی بسازی؛ Render متغیر `PORT` را خودش فراهم می‌کند و سرویس باید روی `0.0.0.0` گوش کند.

### بعد از Deploy این 4 آدرس را تست کن

فرض کنیم Render به تو این آدرس را داده:

```text
https://corevyn-proxy.onrender.com
```

اول:

```text
https://corevyn-proxy.onrender.com/health
```
...
```text
https://corevyn-proxy.onrender.com/api/tse/اهرم
```

بعد:

```text
https://corevyn-proxy.onrender.com/api/codal/اهرم
```

و:

```text
https://corevyn-proxy.onrender.com/api/market
```

**اگر `/health` کار کرد ولی `/api/tse/اهرم` خطای 522/530 داد، دیگر مشکل Cloudflare نیست؛ آن‌وقت دقیقاً می‌فهمیم Render هم از مسیر خودش به TSETMC دسترسی ندارد و باید فقط لایه TSETMC را عوض کنیم.**

نکته مهم: کدی که قبلاً داشتی روی Render هم `app.listen(PORT)` داشت، اما بهتر است حتماً `0.0.0.0` را صریح تعیین کنیم؛ Render همین را برای سرویس‌های Node توصیه می‌کند.

**فعلاً Cloudflare Worker را دست نزن.** ارز فعلی که درست کار می‌کند را نگه می‌داریم. اول Render را سالم می‌کنیم؛ بعد Worker را به همین APIهای Render وصل می‌کنیم و در مرحله بعد دکمه‌های **تحلیل نماد، بورس، کدال، فرصت‌ها و هشدارها** را به همین Backend وصل می‌کنیم.
